...
func blending(p float64,i int, j int, img image.Image,img2 image.Image, wImg image.RGBA){
  for x := 0; x < i; x++ { 
    for y := 0; y < j; y++ { 
      pixel := img.At(x, y) 
...
func main() {  
  ...
  start := time.Now()
  blending(0.5,size.X, size.Y, img,img2, *wImg)
  elapsed := time.Since(start)
  log.Printf("El blending tomo %s", elapsed)
